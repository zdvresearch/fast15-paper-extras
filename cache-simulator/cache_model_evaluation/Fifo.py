...
class FifoCache():
    ...
    def update_obj_size(self, obj_id, size, delta):
        if obj_id in self._cached_objects:
            ...
            self._cached_objects[obj_id] = (obj[CACHE_ATIME], size)
    ...
    def _evict_bytes(self, bytes, xtime):
        ...
        while evicted_bytes < bytes:
            first_obj = self._cached_objects.popitem()[1]
            freed_bytes = first_obj[CACHE_SIZE]
            evicted_bytes += freed_bytes
            self._used_size -= freed_bytes
...
def main(argv=None):
    ...
    cache.cache_object('c', 3300, 30)
    print (json.dumps(cache._cached_objects, indent=2))
    print ("====================================")
    
    cache.cache_object('d', 4400, 40)
    print (json.dumps(cache._cached_objects, indent=2))
    print ("====================================")
    print (json.dumps(cache._cached_objects, indent=2))

    cache.cache_object('e', 5500, 80)
    print (json.dumps(cache._cached_objects, indent=2))
    print ("====================================")
    ...
    cache._remove_cached('b')
    cache._remove_cached('e')
    print (json.dumps(cache._cached_objects, indent=2))
    print ("====================================")
